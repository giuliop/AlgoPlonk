...
func TestAudit(t *testing.T) {
	...
	if tsc.NumG1Powers == 0 {
		t.Fatal("Desired transcript not found")
		return
	}
	...
	if err != nil {
		t.Fatalf("error creating pk.audit file: %v", err)
	}
	defer file.Close()
	...
	if err != nil {
		t.Fatalf("error writing length to file: %v", err)
	}
	...
	for _, g1Power := range tsc.PowersOfTau.G1Powers {
		...
		if len(bytes) != 48 {
			t.Fatalf("decoded hex is not 48 bytes long")
		}

		// Write the 48-byte sequence to the file
		_, err = file.Write(bytes)
		if err != nil {
			t.Fatalf("error writing G1Power to file: %v", err)
		}
	}
	...
	if err != nil {
		t.Fatalf("error creating vk.audit file: %v", err)
	}
	defer file.Close()
	...
	for _, g2Power := range tsc.PowersOfTau.G2Powers[:2] {
		...
		if len(bytes) != 96 {
			t.Fatalf("decoded hex is not 96 bytes long")
		}

		// Write the 96-byte sequence to the file
		_, err = file.Write(bytes)
		if err != nil {
			t.Fatalf("error writing G2Power to file: %v", err)
		}
	}
	...
	if err != nil {
		t.Fatalf("error reading pk.bin: %v", err)
	}
	pkAudit, err := os.ReadFile("pk.audit")
	if err != nil {
		t.Fatalf("error reading pk.audit: %v", err)
	}
	if string(pk) != string(pkAudit) {
		t.Fatalf("pk.bin and pk.audit files do not match")
	}
	vk, err := os.ReadFile("vk.bin")
	if err != nil {
		t.Fatalf("error reading vk.bin: %v", err)
	}
	vkAudit, err := os.ReadFile("vk.audit")
	if err != nil {
		t.Fatalf("error reading vk.audit: %v", err)
	}
	if string(vk) != string(vkAudit) {
		t.Fatalf("vk.bin and vk.audit files do not match")
	}
